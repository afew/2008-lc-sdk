...
INT CGameSgmnt::Query(char* sCmd, void* pData)
{
	...
	else if(0==_stricmp("Load Character", sCmd))
	{
		...
		PDEV			pDev= (PDEV)LcDev_GetD3Device((ILcDev**)&m_pDevSys);
		INT				hr	= Lcm::Mdl_Create("Acm", &pMdl, pOrg->pIns, NULL, (PDEV)pDev);
	...
	{
		Lcm::ILcmMdl*	pMdl = (Lcm::ILcmMdl*)pGameData->GetMateModel(0, 0);
		*((Lcm::ILcmMdl**)pData) = pMdl;
