...
void CLcxmBlc::RenderBlc()
{
	...
	m_pDev->SetFVF(Lcxmo::VtxSpl::FVF);
...
void CLcxmBlc::RenderTline()
{
	...
	pLine[0] = Lcxmo::VtxD(Px+ 0, Py, Pz+ 0, 0xFFFFFF00);
	pLine[1] = Lcxmo::VtxD(Px+ 0, Py, Pz+fW, 0xFFFFFF00);
	pLine[2] = Lcxmo::VtxD(Px+fW, Py, Pz+fW, 0xFFFFFF00);
	pLine[3] = Lcxmo::VtxD(Px+fW, Py, Pz+ 0, 0xFFFFFF00);
	pLine[4] = pLine[0];
	...
	m_pDev->DrawPrimitiveUP(D3DPT_LINESTRIP, 4, pLine, sizeof(Lcxmo::VtxD));
...
void CLcxmBlc::CreateMesh(INT iNx, FLOAT fWx, FLOAT fUV)
{
	...
	m_iNv	= m_iNx * m_iNx;
	m_FVF	= Lcxmo::VtxSpl::FVF;
	m_iVs	= sizeof(Lcxmo::VtxSpl);
	m_pVtx	= new Lcxmo::VtxSpl[m_iNv];
	m_pVtt	= new Lcxmo::SplIdx[m_iNv];
...
void CLcxmBlc::LayerSetup(Lcxmo::lsMpTex* pTxLst)
{
	int iSize = pTxLst->size();

	Lcxmo::lsMpTex::iterator _F = pTxLst->begin();
	Lcxmo::lsMpTex::iterator _L = pTxLst->end();
...
void CLcxmBlc::LayerAdd(Lcxmo::MpTex* pTex)
{
...
void CLcxmBlc::SetIndex()
{
	...
	for(z=0; z< iN/2;++z)														// Index¸¦ Ã¤¿î´Ù.
	{
		for(x=0;x<iN/2;++x)
		{
			...
			for(int m=0; m<8; ++m)
				m_pFce[i+m] = Lcxmo::VtxIdx( f[8], f[(m+1)%8], f[(m+0)%8]);
		}
	}
}
...
void CLcxmBlc::CalculateMapTile(int nTx)
{
	...
	for (z=0; z<m_iNx; ++z)
	{
		for (x=0; x<m_iNx; ++x)
		{
			...
			memcpy(&_color, &m_pVtx[m_iNx*z + x].ts[0], sizeof(Lcxmo::VEC6));

			_color[nTx] = fAlpha;

			memcpy(&m_pVtx[m_iNx*z + x].ts[0], &_color, sizeof(Lcxmo::VEC6));
		}// for
	}// for
}
