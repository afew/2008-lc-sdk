...
LscMdl::ILcCont::~ILcCont()
{
	if(!vLscH.empty())
	{
		...
		{
			ILcmMdl* pDst = (ILcmMdl*)(vLscH[i]->pIns);
			delete pDst;
...
INT LscMdl::ILcCont::Alloc(char* sName, void* pLscHorg)
{
	...
	ILcmMdl*	pOrg = (ILcmMdl*)_t->pIns;
	PDEV		pDev = (PDEV)LscMdl::m_pDevD3D;

	hr = LcMdl_Create("Acm", &pObj, pOrg, sName, pDev);
...
INT LscMdl::ILcCont::AllocOrg(char* sName, char* sTxFolder, char* sEft)
{
	...
	hr = LcMdl_Create("Acm", &pObj, NULL, sName, pDev, pEft);
