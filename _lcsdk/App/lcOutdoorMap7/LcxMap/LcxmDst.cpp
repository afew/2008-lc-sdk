...
INT CLcxmDst::Query(char* sCmd, void* pData)
{
	...
	{
		memcpy(&m_Hgt, pData, sizeof(ILcxmT::MpHgt));
		return 0;
	}
	...
	{
		memcpy(&m_Inf, pData, sizeof(ILcxmT::MpInf));
		return 0;
	}

	else if(0==_stricmp("Get Info", 			sCmd))
	{
		memcpy(pData, &m_Inf, sizeof(ILcxmT::MpInf));
		return 0;
	}

	else if(0==_stricmp("Set Environment", 		sCmd))
	{
		memcpy(&m_Env, pData, sizeof(ILcxmT::MpEnv));
		return 0;
	}

	else if(0==_stricmp("Get Environment", 		sCmd))
	{
		memcpy(pData, &m_Env, sizeof(ILcxmT::MpEnv));
		return 0;
	}
	...
	{
		struct _T{	ILcxmT::MpInf* I; ILcxmT::MpEnv* E; ILcxmT::lsMpTex* L;};
		_T* t = (_T*)pData;
	...
	{
		memcpy(&m_Env.m_Fog, pData, sizeof(ILcxmT::MpFog));
		return 0;
	}

	else if(0==_stricmp("Get Fog", 				sCmd))
	{
		memcpy(pData, &m_Env.m_Fog, sizeof(ILcxmT::MpFog));
		return 0;
	}
	...
	else if(0==_stricmp("Set Light", 			sCmd))
	{
		...
		_T*	t =(_T*)pData;
		m_Env.m_iLg = t->n;
		memcpy(m_Env.m_pLg, t->p, sizeof(ILcxmT::MpLgt) * t->n);
		return 0;
	}

	else if(0==_stricmp("Set Material", 		sCmd))
	{
		memcpy( &m_Env.m_Mtl, pData, sizeof(ILcxmT::MpMtl) );
		return 0;
	}
...
INT CLcxmDst::PickPosition(	CLcxmBlc**			pBlock
						,	ILcxmT::lsTlPkT*	pPickInf	// Picking Information
							...
							)
{
	...
	D3DXVECTOR3		vcPick	;
	ILcxmT::lsTlPkT	vPkLc	;			// Picking Triangle from Local
		...
		for(x=0; x<m_Inf.iNfX; ++x)
		{
			...
			for(i=0; i<pLcl->m_iNi; ++i)
			{
				...
				if(SUCCEEDED(LcMath_LineToTriangle(&vcPk, &fD, pvcRayPos, pvcRayDir, &p0, &p1, &p2)))
					vPkLc.push_back(ILcxmT::MpPckT(vcPk, p0, p1, p2, fD));
			}
		}
	}
	...
	std::sort(vPkLc.begin(), vPkLc.end(), TsrtL<ILcxmT::MpPckT >());
	vcPick = vPkLc[0].vcPk;
...
void CLcxmDst::SetupHeight()
{
	...
	std::vector<ILcxmT::MpVec3T >	vVtxT;
	...
	for(z=0; z<m_Inf.iNfZ; ++z)
	{
		for(x=0; x<m_Inf.iNfX; ++x)
		{
			for(k=0; k<m_ppLcl[z][x].m_iNv; ++k)
			{
				...
				if( iRT<iRng)
				{
					vVtxT.push_back(ILcxmT::MpVec3T(z, x, k, m_ppLcl[z][x].m_pVtx[k].p));
				}
			}
...
void CLcxmDst::SetupTile()
{
	...
	for(z1=nLclBgnZ; z1<=nLclEndZ; ++z1)
	{
		...
		{
			CLcxmBlc*		pLcl = &m_ppLcl[z1][x1];
			ILcxmT::VtxSpl*	pVtx = pLcl->m_pVtx;
...
void CLcxmDst::RenderPck()
{
	...
	if(!m_vPkLc.empty())
	{
		...
		m_pDev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 1, pVtx, sizeof(ILcxmT::VtxD));
...
void CLcxmDst::CreateField(ILcxmT::MpInf* Inf, ILcxmT::MpEnv* Env, ILcxmT::lsMpTex* pTxLst)
{
	if(m_ppLcl)
	{
		for(int z=0; z<m_Inf.iNfZ; ++z)
	...
	memset(&m_Inf, 0, sizeof(ILcxmT::MpInf));
	memset(&m_Env, 0, sizeof(ILcxmT::MpEnv));
		
	memcpy(&m_Inf, Inf, sizeof(ILcxmT::MpInf));
	memcpy(&m_Env, Env, sizeof(ILcxmT::MpEnv));
...
INT CLcxmDst::FileLoad(char* sFile)
{
	...
	fread(&m_Inf, 1, sizeof(ILcxmT::MpInf), fp);
	fread(&m_Env, 1, sizeof(ILcxmT::MpEnv), fp);
	...
	for(z=0; z<m_Inf.iNfZ; ++z)
	{
		for(x=0; x<m_Inf.iNfX; ++x)
		{
			...
			pLc->m_pFce	= new ILcxmT::VtxIdx  [pLc->m_iNi];
			pLc->m_pVtx	= new ILcxmT::VtxSpl[pLc->m_iNv];
			fread(pLc->m_pFce, pLc->m_iNi, sizeof(ILcxmT::VtxIdx), fp);
			fread(pLc->m_pVtx, pLc->m_iNv, sizeof(ILcxmT::VtxSpl), fp);
	...
	for(z=0; z<m_Inf.iNfZ; ++z)
	{
		for(x=0; x<m_Inf.iNfX; ++x)
		{
			...
			for(int n=0; n<iSize; ++n)
			{
				ILcxmT::MpTex*	pLayer = &pLc->m_vTex[n];
...
INT CLcxmDst::FileSave(char* sFile)
{
	...
	fwrite(&m_Inf, 1, sizeof(ILcxmT::MpInf), fp);
	fwrite(&m_Env, 1, sizeof(ILcxmT::MpEnv), fp);
	...
	for(z=0; z<m_Inf.iNfZ; ++z)
	{
		for(x=0; x<m_Inf.iNfX; ++x)
		{
			...
			fwrite(pLc->m_pFce, pLc->m_iNi, sizeof(ILcxmT::VtxIdx), fp);
			fwrite(pLc->m_pVtx, pLc->m_iNv, sizeof(ILcxmT::VtxSpl), fp);
			...
			for(i=0; i<iSize; ++i)
			{
				ILcxmT::MpTex*	pLayer = &pLc->m_vTex[i];
...
ILcxmT::MpInf* CLcxmDst::GetFldInf()
{
	return &m_Inf;
}
