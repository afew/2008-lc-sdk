...
LPDIRECT3DDEVICE9	CD3DApplication::GetDxDevice()	{	return m_pd3dDevice;	}
LPD3DXSPRITE		CD3DApplication::GetDxSprite()	{	return m_pd3dSprite;	}
LPDIRECT3DSURFACE9	CD3DApplication::GetDxSurface()	{	return m_pBackBuffer;	}

void	CD3DApplication::SetClsName(char* sN)	{	strcpy(m_sClsName, sN);		}
BOOL	CD3DApplication::IsWindow()		{	return m_bWindowed;		}
BOOL	CD3DApplication::IsActive()		{	return m_bActive;		}
BOOL	CD3DApplication::GetDxLoadingRnd(){	return m_bLoadingRnd;	}

HWND	CD3DApplication::GetDxHwnd()	{	return m_hWnd;						}
DWORD	CD3DApplication::GetDxClearMode(){	return m_dClearMode;				}
UINT	CD3DApplication::GetDxBackW()	{	return m_d3dsdBackBuffer.Width;		}
UINT	CD3DApplication::GetDxBackH()	{	return m_d3dsdBackBuffer.Height;	}
DWORD	CD3DApplication::GetDxBackF()	{	return m_d3dsdBackBuffer.Format;	}

DWORD	CD3DApplication::GetDxDepthFMT(){	return m_d3dSettings.DepthStencilBufferFormat();	}
...
CD3DApplication::CD3DApplication()
{
	...
	memset(m_sClsName, 0, sizeof m_sClsName);
	strcpy(m_sClsName, "LnD3D Application");

	m_dScnPosX						=-1;
	m_dScnPosY						=-1;

	m_dwCreationW					= 1024;
	m_dwCreationH					= DWORD(3 * m_dwCreationW * 0.25F);
	...
	m_dClearMode					= D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER;
	m_dClearMode |= (m_d3dEnum.AppMinStencilBits>0)? D3DCLEAR_STENCIL: 0;
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
	...
	if( m_hWnd == NULL)
	{
		...
		wndClass.hIcon		= LoadIcon( hInstance, MAKEINTRESOURCE(m_dHicon) );
		wndClass.hCursor	= LoadCursor( NULL, IDC_ARROW );
		wndClass.hbrBackground=(HBRUSH)GetStockObject(WHITE_BRUSH);
		wndClass.lpszMenuName= NULL;
		wndClass.lpszClassName=m_sClsName;
		...
		RECT rc;
		SetRect( &rc, 0, 0, m_dwCreationW, m_dwCreationH );
		AdjustWindowRect( &rc, m_dwWindowStyle, TRUE );

		int iScnSysW = ::GetSystemMetrics(SM_CXSCREEN);
		int iScnSysH = ::GetSystemMetrics(SM_CYSCREEN);

		int iScnSysX = (iScnSysW - (rc.right-rc.left))/2;
		int iScnSysY = (iScnSysH - (rc.bottom-rc.top))/2;

		if(-1 != m_dScnPosX)	iScnSysX = m_dScnPosX;
		if(-1 != m_dScnPosY)	iScnSysY = m_dScnPosY;
		...
		m_hWnd = CreateWindow(
			...
			, (rc.bottom -rc.top)
			, 0
		,	LoadMenu( hInstance, MAKEINTRESOURCE(m_dHmenu) ),
			hInstance, 0 );
	}
...
//			pdm->RefreshRate == bestAdapterDesktopDisplayMode.RefreshRate )
		if( pdm->Width == this->m_dwCreationW &&
			pdm->Height == this->m_dwCreationH && 
			pdm->RefreshRate == bestAdapterDesktopDisplayMode.RefreshRate )
		{
...
//		hMenu = GetMenu(hWnd);
		...
		if( hMenu != NULL && (void*)hMenu !=(void*)0xCCCCCC && (void*)hMenu !=(void*)0xCDCDCDCD)
			DestroyMenu( hMenu );
...
INT CD3DApplication::Run()
{
	...
	HACCEL hAccel = LoadAccelerators( NULL, MAKEINTRESOURCE(m_dHaccel) );
...
void CD3DApplication::UpdateStats()
{
	...
	if( fTime - fLastTime > 1.0f )
	{
		...
		_sntprintf( m_strFrameStats, cchMaxFrameStats, _T("(%dx%d), %s%s%s fps:%.f")
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
	...
	if( MSGERR_APPMUSTEXIT == dwType )
	{
		...
		MessageBox( NULL, strMsg, m_sClsName, MB_ICONERROR|MB_OK );
	...
	else
	{
		...
		MessageBox( NULL, strMsg, m_sClsName, MB_ICONWARNING|MB_OK );
	}
...
HRESULT CD3DApplication::ConfirmDevice(D3DCAPS9* pCaps,DWORD dwBehavior,DWORD Format)
{
