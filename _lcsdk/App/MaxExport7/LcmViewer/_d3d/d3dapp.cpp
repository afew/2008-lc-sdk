...
CD3DApplication::CD3DApplication()
{
	...
	strcpy(m_sClassName, "D3D Window");
...
bool CD3DApplication::ConfirmDeviceHelper( D3DCAPS9* pCaps, DWORD vertexProcessingType, DWORD backBufferFormat )
{
	DWORD dwBehavior;
...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
	...
	if( m_hWnd == NULL)
	{
		...
		WNDCLASS wndClass =
		{
			...
			, 0
			, hInstance
			, LoadIcon( hInstance, MAKEINTRESOURCE( m_dRscIcon ) )
		...
		m_hWnd = CreateWindow( m_sClassName, m_sClassName, m_dwWindowStyle
			,	m_dScnX			// (iScnSysW - (rc.right-rc.left))/2
			,	m_dScnY			// (iScnSysH - (rc.bottom-rc.top))/2
			,	(rc.right-rc.left)
			,	(rc.bottom-rc.top)
			, 0,
			LoadMenu( hInstance, MAKEINTRESOURCE( m_dRscMenu ) ),
			hInstance, 0 );
	}
...
//	for(idm = 0; idm < pBestAdapterInfo->pDisplayModeList->Count(); idm++ )
//	{
								 ...
								 LPARAM lParam )
{
	...
	case WM_SETCURSOR:
		ShowCursor(m_bShowCursorWhenWindow);
...
#define PBT_APMRESUMESUSPEND 0x0007
		...
		case WM_COMMAND:
		{
			...
			if(m_dRscDevice && m_dRscDevice == wLo )
			{
			...
			if(m_dRscToggle && m_dRscToggle == wLo )
			{
				Pause( true );
...
HRESULT CD3DApplication::Initialize3DEnvironment()
{
	...
	if( SUCCEEDED(hr) )
	{
		...
		if(m_pd3dSfBack)
			m_pd3dSfBack->Release();

		if(FAILED(m_pd3dDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &m_pd3dSfBack)))
			return-1;
		m_pd3dSfBack->GetDesc( &m_d3dsdBackBuffer );
...
INT CD3DApplication::Run()
{
	...
	HACCEL hAccel = LoadAccelerators( NULL, MAKEINTRESOURCE( m_dRscAccel ) );
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
	...
	if( MSGERR_APPMUSTEXIT == dwType )
	{
		...
		MessageBox( NULL, strMsg, m_sClassName, MB_ICONERROR|MB_OK );
	...
	else
	{
		...
		MessageBox( NULL, strMsg, m_sClassName, MB_ICONWARNING|MB_OK );
	}
